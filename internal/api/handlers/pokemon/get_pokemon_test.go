...
func TestGetPokemon(t *testing.T) {
	test.WithTestServer(t, func(s *api.Server) {
		...
		pokemonName := fixtures.PokemonNotInCollection.Name
		res = test.PerformRequest(t, s, "GET", "/api/v1/pokemon?name="+pokemonName, nil, test.HeadersWithAuth(t, fixtures.User1AccessToken1.Token))
		require.Equal(t, http.StatusOK, res.Result().StatusCode)

		var nameFilterResponse types.GetPokemonResponse
		err = json.NewDecoder(res.Result().Body).Decode(&nameFilterResponse)
		require.NoError(t, err)

		// check that only the specific pokemon was returned
		require.Len(t, nameFilterResponse.Data, 1)
		require.Equal(t, pokemonName, *nameFilterResponse.Data[0].Name)
